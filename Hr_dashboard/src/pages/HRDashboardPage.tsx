...
export const HRDashboardPage: React.FC = () => {
  ...
  const [cacheSuccessMsg, setCacheSuccessMsg] = React.useState<string | null>(null);

  const clearCacheMutation = useMutation({
    mutationFn: () => apiRequest<{ message: string; flushedKeysCount: number }>('/hr/cache/clear', { method: 'POST' }),
    onSuccess: (res) => {
      setCacheSuccessMsg(`Redis cache cleared (${res.flushedKeysCount} keys flushed)`);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {cacheSuccessMsg && (
        <div className="p-3 bg-emerald-500 text-slate-950 font-bold text-xs rounded-xl shadow-lg flex items-center justify-between animate-fade-in">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4" />
            <span>{cacheSuccessMsg}</span>
          </div>
          <button onClick={() => setCacheSuccessMsg(null)} className="text-xs opacity-75 hover:opacity-100">
            Dismiss
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-slate-100 border border-slate-200 px-3.5 py-2 text-xs font-bold text-slate-800 shadow-xs hover:bg-amber-50 hover:text-amber-900 hover:border-amber-300 transition disabled:opacity-50"
            title="Flush Upstash Redis cache and in-memory caches"
          >
            <RotateCcw className={`h-3.5 w-3.5 text-amber-600 ${clearCacheMutation.isPending ? 'animate-spin' : ''}`} />
            <span>{clearCacheMutation.isPending ? 'Flushing...' : 'Flush Redis Cache'}</span>
          </button>
