...
import { Shield, Sparkles, LogOut, User, Menu, ChevronDown, Bell } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar }) => {
  ...
  const displayName = user?.username || 'HR Administrator';
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 sm:px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3">
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200/80 text-amber-900 text-xs font-semibold">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          <span>HR Admin Scope</span>
        </div>
        ...
        {user ? (
          <div className="relative">
            ...
            >
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-900 text-amber-400 text-xs font-bold font-display">
                {displayName.charAt(0).toUpperCase()}
              </div>
              <div className="hidden md:block">
                <span className="block text-xs font-bold text-slate-800 leading-tight">{displayName}</span>
                <span className="block text-[10px] font-medium text-slate-500 leading-tight">Human Resources</span>
              </div>
